  ...
  async function fetchLivePrice() {
    ...
    try {
      if (category === 'stock') {
        ...
        const data = await fetchCryptoMarketData([id]);
        if (data[id]) {
          setLivePrice(data[id].price);
          setLogoUrl(data[id].image);
        }
    ...
    <SafeAreaView style={styles.safe}>
      <ScrollView keyboardShouldPersistTaps="handled">
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Tambah aset</Text>
        </View>
        ...
        <TextInput style={styles.input} placeholder="e.g. BBCA / Bitcoin / BCA Tabungan" value={name} onChangeText={setName} placeholderTextColor={colors.textMuted} />
        ...
        <TextInput style={styles.input} placeholder="BCA / Stockbit / Indodax" value={institution} onChangeText={setInstitution} placeholderTextColor={colors.textMuted} />

        {/* Quantity / Value */}
        {category === 'cash' ? (
          <>
            <Text style={styles.label}>Saldo (IDR)</Text>
            <TextInput style={styles.input} placeholder="Rp 0" keyboardType="numeric" value={manualValue} onChangeText={setManualValue} placeholderTextColor={colors.textMuted} />
          </>
        ) : (
          <View style={styles.twoCol}>
            <View style={{ flex: 1 }}>
              <Text style={styles.label}>{category === 'stock' ? 'Jumlah lot' : 'Jumlah unit/koin'}</Text>
              <TextInput style={styles.input} placeholder="0" keyboardType="decimal-pad" value={quantity} onChangeText={setQuantity} placeholderTextColor={colors.textMuted} />
            </View>
            <View style={{ flex: 1 }}>
              <Text style={styles.label}>Harga beli (IDR)</Text>
              <TextInput style={styles.input} placeholder="Rp 0" keyboardType="numeric" value={buyPrice} onChangeText={setBuyPrice} placeholderTextColor={colors.textMuted} />
            </View>
          </View>
        )}
